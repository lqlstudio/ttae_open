...
//API检查错误
function top_check_error($resp,$show){
		global $_G;
			...
			$show = true;
			if(in_array(CURSCRIPT,array('fetch','api','app','app2')))$show = false;
...
function get_prize_num(){
		global $_G;
		if(SYSTEM_TYPE<3) return false;
		foreach($_G[prize] as $k=>$v){
			$re[$k] = getcount('ticket',"prizeid = ".$v[id]." AND is_use=0");
		}
		return $re;
...
function reg($user){
	...
	if($_GET[type] == 'seller')$arr[groupid] = 4;
...
function get_sign_jf($uid){
		...
		$rs = DB::fetch_all("SELECT dateline FROM ".DB::table('sign')." WHERE uid=".$uid ." AND type='sign' ORDER BY id DESC LIMIT ".$size);

		$sign_days =0;		
		$i=1; 		

		//今天如果签到了就从0开始 
		$today = dgmdate($rs[0][dateline],'Ymd');
		if(dgmdate(TIMESTAMP,'Ymd') == $today)$i=0;

		foreach($rs as $k=>$v){
			$t =dgmdate($v[dateline],'Ymd');
			if($t==dgmdate(TIMESTAMP-($i*86400),'Ymd')){
				$sign_days++;
			}else{
				break;
			}
		...
		//if(dgmdate(TIMESTAMP,'Ymd') == $today)$sign_days++;
...
function api_post($parent){
	global $_G;
	if(defined('API') || API ===true) return false;	//死循环
	...
	$link = "/fetch/api.php?syn_key=".md5($_G['setting']['syn_key']);
	if(!$parent['m']) $parent['m'] = "post";	
	$parent['id'] = (string) $parent['id'];
	if($parent['a'] == 'insert' && $parent['id']) $parent['data']['id'] = $parent['id'];
	$data = array();
	if($parent['data'] && is_array($parent['data'])){
		foreach($parent['data'] as $k=>$v){
			$data[$k] = urlencode_utf8($v);
		}	
		$parent['data'] = json_encode($data);
	}
		...
		if(!$url) return false;
		 ...
		 if($use_class){
				include_once libfile('class/curl');
				 $curl = new Curl($cookie_file);
				// $curl->debug=true;
				try{
				 	if (is_array ( $parameters ) && count ( $parameters )>0) {
						$rs=  $curl->post($url, $post_data, $referer);
				 	}else{
						$rs= $curl->get($url); 
				 	}
				}catch(Exception $e){
...
function is_login($not_reg=false){
		global $_G;
		if(!$_G[uid]){
