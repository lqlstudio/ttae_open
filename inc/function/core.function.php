...
function msg($message,$status,$url,$ext_msg){
	global $_G;	
	define('ERROR',true);
...
function D($arr,$page){
	...
	$all 		= 		$arr['all']		?  true :false;
	$table 		= 		$arr['table']	?  $arr[table]: 'goods';
	$time 		= 		$arr['time']	?  $arr['time'] : intval($_G['setting']['cache_time'])*60;		//默认5分钟
	
	//$key 		= 		$arr['key']		?  CURMODULE.'_'.CURACTION.'_'.$table.'_'.$_G[page].'_'.$arr['key'] : '';
	if($arr['key']){
		if($page['url']){
			$key = CURMODULE.'_'.CURACTION.'_'.$arr['key'].'_'.$_G['page'].md5($page['url']);
		}else{
			$key = CURMODULE.'_'.CURACTION.'_'.$arr['key'].'_'.$_G['page'].'_'.$table.'_'.$_G[page];
		}
	}
	$order = '';
	if($arr['order']){
	...
	if(	$table == 'goods'&& !defined('IN_ADMIN') && $all==false) {		
	...
	if($key && $_G['setting']['cache_time']>0) memory('set',$key,$result,$time);
	return $result;
...
function get_keywords2($title,$length){
			global $_G;
			if(!$title) return '';
			$title = urldecode($title);
			$title = trim_html($title,1);
			$num = $length? intval($length) : 10;
...
function get_keywords($title='',$content=''){
	global $_G;
		...
		if($_G['setting'][first_keywords]){
			...
			$default = array();
			foreach($kw as $k=>$v){
				if(strpos($org_content,$v) !==false)$default[] = $v;
			}
...
function load_excel($file,$parse_arr = true,$del = false){
	include_once ROOT_PATH."inc/api/excel_reader2.php";
	
	$load_file = ROOT_PATH.$file;
