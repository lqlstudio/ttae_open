...
class upload{
	...
	function init($attach, $type = 'temp', $extid = 0, $forcename = '') {
		...
		if(!is_array($attach) || empty($attach) || !$this->is_upload_file($attach['tmp_name']) || trim($attach['name']) == '' || $attach['size'] == 0) {
			...
			$attach['target'] = ROOT_PATH.$this->type.'/'.$attach['attachment'];
			$this->attach = & $attach;
			$this->errorcode = 0;
			return true;
		}
	...
	function get_target_extension($ext) {
		static $safeext  = array('attach', 'jpg', 'jpeg', 'gif', 'png', 'swf', 'bmp', 'txt', 'zip', 'rar', 'mp3','doc','docx','xlsx','xls','psd');
	...
	function check_dir_type($type) {
		...
		$dir = ROOT_PATH.$type;
			if(!is_dir($dir)){
				if(!dmkdir($dir)) echo '鍒涘缓鐩綍澶辫触!'.$dir;
			}
