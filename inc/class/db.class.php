...
class DB {
	...
	public static function init(&$db,$pre) {
		self::$db =& $db;
		self::$pre = $pre;
		//self::$db->connection();
	}
	...
	public static function out($out = true, $all = true) {
		return self::$db->sqlOutput($out, $all);
	}
	...
	public static function insert($table, $data, $return_insert_id = false, $replace = false, $silent = false) {
		...
		self::query("$cmd $table SET $sql", null, $silent, !$return_insert_id);
	...
	public static function fetch_all($sql,  $keyfield = '',$arg = array(), $silent=false) {
	...
	public static function num_rows($resourceid) {
		return self::$db->affected_rows($resourceid);
	}
	...
	public static function checkquery($sql) {
		return _safecheck::checkquery($sql);
	}
