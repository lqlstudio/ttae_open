<?php
if(!defined('IN_TTAE')) {
	exit('Access Denied From IN_TTAE');
}
...
class sql {
	...
	function __construct(){
		...
		$this->complete = true;
			...
			function insert_sql($content,$replace){
				
				
				if(!$content) return false;
				$Charset = str_replace('-','',CHARSET);
				if($replace){
					$readfiles=str_replace('$timestamp',"$timestamp",$content);
				}
			...
			function insert_file($file,$replace=''){				
				...
				return $this->insert_sql($readfiles,$replace);
