	...
	function taobao_login(){
				...
				if(isset($_GET["state"]) && !empty($_GET["state"])){
						...
						$postfields = array ('grant_type' => "authorization_code",'client_id' => $_G[setting][taobao_appkey],'client_secret' => $_G[setting][taobao_appsecret],
								'code' => $code,'redirect_uri' => $callbak_url);
						...
						$member = array('username'=>urldecode_utf8($info[taobao_user_nick]),'address'=>'','content'=>$info[taobao_user_id],
						'picurl'=>'','groupid'=>22,'login_name'=>'taobao','login_id'=>$info[taobao_user_id]);
						$this->login_callback($member);
			...
			global $app,$_G;
				...
				if($_G[uid])msg('您已登录,请退出后再进行操作','error','m=index');
						...
						if( $_GET[username]&& $_GET[login_submit] && check()){
								...
								if($_GET[type] == 'seller' && $user[groupid] != 4){
								...
								if(!$user['password'] && $user['login_id']){
								 ...
								 if($user[uid]>0 && authcode($user[password],'decode',$user['key']) ==$password ){							   
									...
									msg('用户不存在或密码不正确','error','m=member&a=login');
								}		
						}
								  ...
								  if($count<=$_G[setting][yaoqing_num] && $count_day <=$_G[setting][yaoqing_day]  ){
										...
										if($tmp[uid]>0 && $tmp[groupid] != 3 && $tmp[check] ==1){
											...
											$sid =insert_sign(array('uid'=>$tmp[uid],'username'=>$tmp[username],'desc'=>$tmp[username].'邀请'.$arr[username].'注册',
														'type'=>'yaoqing','org_jf'=>$add_jf,'jf'=>$jf)
											);
											
											//自动升级到推广组
											if($tmp['rank'] == 0 && $_GET['setting']['fanli'] ==1 ) $update_arrr['rank'] = 1;
											if($sid){
					...
					if($_GET['new_password'] ==1){
						if(!$_GET[password1] || !$_GET[password2]){
							...
							if(!$user[uid]){
								...
								$arr = array();
								$arr['key'] = random(10);
								$arr[password] = authcode($new_password,'encode',$arr['key']);
