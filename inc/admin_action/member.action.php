...
class member extends app{	
	...
	function post(){
					...
					if($_GET['onsubmit'] && check() ){
							...
							if($member['org_login_time'])$member['login_time'] = dgmdate($member['org_login_time'],'dt');
							if($member['org_regdate'])$member['regdate'] = dgmdate($member['org_regdate'],'dt');
							$login_type = array('qq'=>'qq','weibo'=>'微博','taobao'=>'淘宝');
							if($member['end_time'])$member['end_time'] = dgmdate($member['end_time'],'dt');
	...
	function rank(){
			global $_G;
			  ...
			  /*if($_GET['onsubmit'] && check() ){
					  foreach($_GET[ids] as $k=>$v){
						  ...
						  if($_GET[del][$k] ==0) continue;
						  if($_GET['_del_all']==1 && $_GET['del'][$k]){									
							  DB::delete('rank','id='.$id);
							  DB::update('member',array('rand'=>0),'rank='.$id);
						  }
					  }
					  cpmsg('操作成功','success','m='.__CLASS__.'&a='.__FUNCTION__);
					  
			  }*/
			$rs = D(array('table'=>'rank','and'=>'','order'=>' id ASC'),array('url'=>'m=member&a=rank','size'=>40));
			$this->add($rs);
			$this->show();
	}
		function rank_post(){
			 ...
			 if($_GET['onsubmit'] && check() ){
							$arr = array();
							$arr['name'] = $_GET['postdb']['name'];	
							$arr['bili'] = intval($_GET['postdb']['bili']);
							/*
							$arr['jf_min'] = intval($_GET['jf_min'])	;
							$arr['jf_max'] = intval($_GET['jf_max'])	;
							*/
					...
					loadcache('rank','update');
					cpmsg( $msg.'成功','success','m='.__CLASS__.'&a='.__FUNCTION__."&id=".$_G[id]);
			 }else if($_GET['id']){
				$rank =  DB::fetch_first("SELECT * FROM ".DB::table('rank')." WHERE id = ".$_G['id']);
				$this->add(array('rank'=>$rank));
			}
			$this->show();
		}
		...
		function rank_del(){
					global $_G;
					if(!$_GET['id']){
						 cpmsg('抱歉,要删除的推广组不存在','error',"m=member&a=rank");
						 return false;
					}
					$id = intval($_GET['id']);
					if(!$_GET['ok']){
						cpmsg('您确定要删除当前的推广组吗?删除后不可恢复?','error',"m=member&a=rank_del&ok=1&id=".$id,'确定删除',"<p><a href='".URL."m=member&a=rank'>取消</a></p>");
						return false;
					}else{
						DB::delete('rank','id='.$id);
						DB::update('member',array('rank'=>0),'rank='.$id);
						cpmsg('删除成功','success',"m=member&a=rank");
						return false;
					}
	}
