...
class api_m_taobao {
		function update_goods($arr,$aid,$posttime){
				...
				if($aid>0){
					...
					//肯定是没有获取成功
					if(($arr[sum] == 0 && $arr[num] == 0) ) return false;
...
private		function parse_goods($arr){
			...
			$tmp['sid'] 		= 		$arr['seller']['userNumId'];
			$tmp['nick'] 		= 		$arr['seller']['nick'];
			$tmp['hide'] 		= 		$arr['itemInfoModel']['saleLine'] =='online'?0:1;
			...
			$location = $arr['itemInfoModel'][location];			
			$tmp[city] 		= 		mb_substr($location,0,2,CHARSET);		
			$tmp[state] 	= 		mb_substr($location,2,4,CHARSET);
			if(TAE)$content = unicode_decode($content,'UTF-8',"\xe");
			...
			$tmp[baoyou] = $tmp2[data][delivery][deliveryFees][0]== '卖家包邮' ?1:0;
