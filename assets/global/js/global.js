...
function Marquee2(cf){
		...
		if(typeof obj.next == 'string' && obj.next){
				click(obj.box +' '+obj.next,function(){
		...
		if(obj.bar){
			...
			if(!obj['len'] || obj['len'] =='' || obj['len'] <1){
				obj.len = $(obj.box +' '+obj.run_box).children().length;
...
if($('.show_menu,.showMenu').length>0){
	
	
	$('.show_menu,.showMenu').each(function(i){
			...
			if(config){
				...
				$(this).click(function(){
					...
					if(id)showMenu({ctrlid:id,pos:'!00',zindex:2,'duration':3});
					return false;
				})
			}
	});
...
function add_favorite(obj,call){
	if(typeof obj != 'object') obj = this;
	var aid = $(obj).attr('data-aid');
	if(!aid)aid =$(obj).attr('data-id');

	if(typeof aid != 'string' || !aid) return false;
	var dtype = $(obj).attr('data-type');
	if(!dtype && $(obj).attr('class').indexOf('like') != -1) dtype = 'like';
	var type = dtype?dtype:'addfavorite';


	
	ajaxget('/index.php?m=ajax&a=add_favorite&id='+aid+'&type='+type,function(s){
...
function m_get_goods(iid,call){
	var url = '/index.php?m=ajax&a=m_get_goods&num_iid='+iid;
	$.ajax({type:'GET',url:url,data:'',success:function(s){
		if(s.status == 'error'){
