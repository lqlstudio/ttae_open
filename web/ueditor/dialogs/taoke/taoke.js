(function () {
    ...
    UM.registerWidget('taoke', {
			...
			$root.find('.edui-tab-findbox .find_button').click(function(){
				...
				if(!iid){
					...
					m_get_goods(iid,function(s){
						...
						get_keywords(s.title,function(s1){
							var s2 = s1.split(',');
		...
		cover_data:function(obj,keyword_arr){
			...
			rs+='<a class="item_url" href="'+url+'" data-num_iid="'+iid+'" isconvert="1" target="_blank" data-sellerid="'+sid+' title="'+obj.title+'" rel="nofollow" ">';
			...
			rs+=' data-sellerid="'+sid+' title="'+obj.title+'" rel="nofollow" ">'+obj.title+'</a>';
			...
			rs+='<a  class="item_url"  href="'+url+'" data-num_iid="'+iid+'" isconvert="1" target="_blank" data-sellerid="'+sid+'  rel="nofollow"  ">去购买</a>';
			rs+='</div></div></div>';
