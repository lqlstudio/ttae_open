...
<div class="bdsharebuttonbox" data-size="24" data-picurl="{$img.picurl}"  data-desc="{$img.description}">
</div>

            </div>
           <span>来源：{$img.from_name}</span><span>发表于 {$img.dateline}</span>
          </div>
        </div>
      </div>
      <!-- /info -->
...
<div class="baidu_share" data-size="32" data-picurl="{$img.picurl}"  data-desc="{$img.description}"></div>
        </dl>
      </div>
      <div class="page">
        <li>上一篇：{$img.up} </li>
...
{/foreach}
	...
	<div class="_duoshuo" data-id="{$img.id}" data-name="{$_G.setting.duoshuo}"></div>
    </div>
  </div>
