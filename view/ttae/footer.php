...
<!--悬浮框-->
<div class="menufixd" style="top: 50px;{if $CURMODULE == 'home'}display:none;{/if}">
    ...
    <h1>商品分类</h1>	
 	  ...
 	  <li class="indexajaxlink" ><a href="{$_G.siteurl}" class="{if $_G.fid ==0}fixedselect{/if}">全部</a></li>
        {foreach from = $_G.channels item=v name= a}
         <li><a href="{$URL}fid={$v.fid}" class="{if $_G.fid == $v.fid }fixedselect{/if}">{$v.name}{$v.name}</a></li>
         {/foreach}
    ...
    <ul class="fixedmenu2_1 cl">
        <li class=""><a href="{$URL}m=duihuan&a=list" class="newindexicon fixednico5">积分中心</a></li>
        <li class=""><a href="{$URL}m=shop&a=list" class="newindexicon fixednico3">品牌团</a></li>
         <li><a  href="{$URL}m=img&a=list" class="newindexicon fixednico4">值得买</a></li> 
    </ul>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="m_c fwin_content_sign"><h3 class="flb"><em>提示</em><span> <a title="关闭" class="fwin_window_close_sign flbc _hideMemu" target="_blank">关闭</a> </span></h3>
          <div class="content_box_0"><div class="_share" data-style="2" data-more="0" data-count="0" data-picurl="{$goods.picurl}" style="  padding: 20px 0 0 100px;"></div></div></td>
        <td class="m_r"></td>
      </tr>
      <tr>
        <td class="b_l"></td>
