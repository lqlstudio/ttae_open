...
<a href="{$_G.siteurl}">首页</a> > <a href="{$URL}fid={$_G.fid}">{$goods.channel.name}</a> > <a href="{$goods.id_url}" class="on">{$goods.title}</a>
</div>
...
<div class="bucuo_detail_con">
   ...
   <div class="bucuo_de_goodsd">
      <ul>
          ...
          <li class="bucuo_de_bkico li2 lic{if $goods.status ==3 || $goods.status ==4}2{elseif $goods.status ==5}3{else}1{/if}">
         
              <b>￥</b><span>{$goods.yh_price}</span>
               <span class="del">原价:{$goods.price}元</span>
              <em>{$_G.setting.title}折扣价</em>
              <a href="{$goods.url}" target="_blank" rel="nofollow" title="{$goods.title}" isconvert="1" data-itemid= "{$v.num_iid}"></a>
          </li>
          <li class="li3">
...
<a href="{$URL}kw={$k}" target="_blank" title="{$v}">{$v}</a>
{/foreach}
        ...
        <li class="li6">{$_G.title} 在{$goods.start_time}进行特惠折扣减价促销活动,原价{$goods.price}现价{$goods.yh_price},开始时间{$goods.start_time}结束时间{$goods.end_time},过后恢复原价.
            ...
            <div class="ai_bucuowk">
                ...
                <div class="ai_bucuoinfo11">
                    <div class="ai_bucuosginfol1">
                     ...
                     <span>
                        ￥<em>{$v.yh_price}</em><del>￥{$v.price}</del><b>（{$v.zk}折）</b>
                     </span>
                    </div>
...
{/foreach}
     ...
     <div class="comment-title">
                      ...
                      <p class="more"><a href="{$goods.url}" target="_blank" rel="nofollow">查看更多评论&gt;&gt;</a></p>
     </div>
         </div>           
