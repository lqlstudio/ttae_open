...
class install{
	private $_config = array('db'=>array(),'cache_list'=>array());
	...
	function __construct(){
				...
				$this->_config['cache_list'] = $_config['cache_list'];
...
private function init_db(){
	...
	function run($type){
		$file = ROOT_PATH.'install/init_config.txt';
		if(!TAE && (!is_file($file) || $type == 'check')) {
			$address = trim($_POST['address']);
			$username = trim($_POST['username']);
			$password = trim($_POST['password']);
			$name = trim($_POST['name']);
			$port = intval($_POST['password']);
		...
		if($exists){
			...
			DB::query("CREATE DATABASE ".$this->_config['db']['dbname']);
			if($type == 'check')	$this->save_config(true);
	...
	function inset_data(){
				...
				$sql = new sql();				
				$len = $sql->insert_file(ROOT_PATH.$file);				
...
private 	function over(){
				memory('clear');
				if(!$this->_config['cache_list'])include  ROOT_PATH.'inc/config/db.config.php';				
				loadcache($this->_config['cache_list'],'update');
